...
function createTreeHeatmap(domElement,root_node,opts){ //
	...
	constants = {
	    ...
	    currency_formatter: d3.format('.3s'),
	};
	...
	function calc_leaf_height() {
	    ...
	    var leaf_height = leaf_height > 34 ? leaf_height : 34;
	...
	return d.leaf && (!chart.show_leaf_text || chart.unit === constants.units.NONE) ? 0.3 : 1;
    }
	...
	if (d.empty)
	    ...
	    return constants.currency_formatter(d.value,0);
     }
	...
	// cell label
	gcell.append('text')
	    .text( function(d) { return d.empty ? '' : d.node.label_short; } )
	    .attr("class",     "label")
	...
	// tooltip.append('rect')
	//     .attr("x", function(d){ return cell_x(d)+6; } )
	//     .attr("y", function(d){ return cell_y(d)+cell_font_size(d)-35;} )
	//     .attr("width",    100)
	//     .attr("height",   23)
	//     .attr("rx",       6)
	//     .attr("ry",       6);
	...
	//     .attr("fill",      "#fff")
	//     .attr("x", function(d){ return cell_x(d)+6; } )
	//     .attr("y", function(d){ return cell_y(d)+cell_font_size(d)-15;} );
	...
	var gtitle;
	chart.container.selectAll('g.slider').remove();
	gslider = d3.slider().on("slide", function(evt, value) {
	...
	// change the depth of the tree to be displayed
	change_labels: function(options) {
	    rebuild_chart();
	},
...
function includeTooltip(){
	...
	var txt = d3.select(this).select('.label').node().textContent;
	txt = txt + ' | ' + d3.select(this).select('.value').node().textContent;
	d3.select(this).attr('label',txt)
	var gTip = d3.select(this).append('g')
	...
	.on("mouseover", function(){
	    tooltip.text(this.getAttribute('label'));
...
function set_changed(e){
    data = loadSet(e.options[e.selectedIndex].value);
    //console.log(data);
    if(data.menu !== "undefined"){
	var menuO = '<option value="'+0+'" unit="NONE">'+"totale"+'</option>'
	for(var i=0;i<data.menu.length;i++){
	    menuO += '<option value="'+(i+1)+'" unit="NONE">'+data.menu[i]+'</option>';
	}
	$('#sizemenu').html(menuO);
    }
    build_parent_links(data, null);
    aggregate_values(data);
    heatmap.change_root_node(data);
    var unitValue = e.options[e.selectedIndex].unit;
    var dwnFile = String(e.options[e.selectedIndex].value);
    $('#dwnFile').html('<span>tabelle:</span><a href="'+ dwnFile.replace('json','csv') +'">completa</a>');
    //d3.select('#chart>svg>g>g').selectAll('text').text(data.title);
...
data = loadSet('data/heatmap_demo.json');
build_parent_links(data, null);
aggregate_values(data);
options = {
    title: data.title,
